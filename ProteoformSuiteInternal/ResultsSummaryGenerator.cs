    ...
    public static class ResultsSummaryGenerator
    {
        ...
        public static string proteoform_families_report()
        {
            ...
            List<string> topdown_ids = Sweet.lollipop.topdown_proteoforms
               .Select(p => p.accession.Split('_')[0].Split('-')[0] + "_" + p.topdown_begin + "_" + p.topdown_end + "_" + string.Join(", ", p.topdown_ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(m => m))).ToList();
                ...
                .ThenBy(e => e.ptm_set.ptm_combination.Count))
            {
                results.Rows.Add(
                    ...
                    e.ptm_set.ptm_combination.Count == 0 ?
                        "Unmodified" :
                        string.Join("; ", e.ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(m => m)),
                    e.begin + " to " + e.end,
                    e.uniprot_mods,
                    e.novel_mods,
                    e.mass_error,
        ...
        public static DataTable topdown_results_dataframe()
        {
            ...
            foreach (TopDownProteoform td in Sweet.lollipop.topdown_proteoforms)
            {
                results.Rows.Add(
                    ...
                    td.linked_proteoform_references == null ? "N/A" : td.ptm_set.ptm_combination.Count == 0 ? "Unmodified" : string.Join("; ", td.ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(m => m)),
                    td.topdown_ptm_description,
                    td.topdown_ptm_set.ptm_combination.Count == 0 ?
                        "Unmodified" : string.Join("; ", td.topdown_ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(m => m)),
