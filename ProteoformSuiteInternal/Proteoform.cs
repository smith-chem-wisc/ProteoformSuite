            ...
            TheoreticalProteoform theoretical_base, int additional_ptm_penalty, bool final_assignment)
        {
            ...
            foreach (PtmSet set in possible_peak_assignments)
            {
                ...
                foreach (ModificationWithMass m in mods_in_set)
                {
                    ...
                    bool motif_matches_n_terminus = !mods_in_set.Any(mod => mod.modificationType == "AminoAcid" && m.motif.ToString() == "M") && this.begin - theoretical_base.begin  >= 0 && this.begin - theoretical_base.begin < theoretical_base.sequence.Length && m.motif.ToString() == theoretical_base.sequence[this.begin - theoretical_base.begin].ToString();
