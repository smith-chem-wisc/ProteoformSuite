        ...
        public List<TopDownHit> ReadTDFile(InputFile file)
        {
            ...
            foreach (List<string> cellStrings in cells)
            {
                ...
                if (add_topdown_hit)
                {
                    ...
                    Convert.ToInt16(cellStrings[17]), Convert.ToDouble(cellStrings[18]), cellStrings[14].Split('.')[0], file.targeted_td_result, Convert.ToDouble(cellStrings[20]), Convert.ToDouble(cellStrings[22]));
                    td_hits.Add(td_hit);
                }
            }
            return td_hits;
