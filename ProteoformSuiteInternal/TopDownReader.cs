            ...
            Parallel.ForEach(cells, cellStrings =>
            {
                ...
                if (tdResultType != TopDownResultType.Unknown) //uknown result type! 
                {
                    ...
                    if (cellStrings[10].Length > 0) //N Terminal Modification Code
                    {
                        ...
                        {
                            ModificationWithMass mod = Sweet.lollipop.theoretical_database.uniprotModifications.Values.SelectMany(m => m).OfType<ModificationWithMass>().Where(m => m.id.Contains("acetyl") && m.motif.Motif == cellStrings[4][0].ToString()).FirstOrDefault();
                            if (mod != null)
                            {
