                        ...
                        foreach (ExperimentalProteoform ep in experimentals)
                        {
                            if (ep.topdown_id)
                            {
                                ...
                                if (tdp.matching_experimental != null)
                                {
                                    ...
                                    string exp_ptm = exp.ptm_set.ptm_combination.Count == 0 ? "Unmodified" : string.Join(", ", exp.ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(p => p));
                                    string td_ptm = tdp.topdown_ptm_set.ptm_combination.Count == 0 ? "Unmodified" : string.Join(", ", tdp.topdown_ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(p => p));
                            ...
                            {
                                string exp_ptm = ep.ptm_set.ptm_combination.Count == 0 ? "Unmodified" : string.Join(", ", ep.ptm_set.ptm_combination.Select(ptm => Sweet.lollipop.theoretical_database.unlocalized_lookup.TryGetValue(ptm.modification, out UnlocalizedModification x) ? x.id : ptm.modification.IdWithMotif).OrderBy(p => p));
