...
namespace Test
{
    class TestFLASHDeconv
    {
        ...
        public void testFLASHDeconv()
        {
            ...
            Assert.IsFalse(File.Exists(filepath + "_decon.tsv"));
            ...
            Assert.AreEqual(633, deconv_components.Count);
            Assert.AreEqual(6999.01, Math.Round(deconv_components.OrderBy(c => c.id).First().reported_monoisotopic_mass, 4));
            Assert.AreEqual(6999.01, Math.Round(deconv_components.OrderBy(c => c.id).First().weighted_monoisotopic_mass, 4));
            Assert.AreEqual(16963900000.00, Math.Round(deconv_components.OrderBy(c => c.id).First().intensity_reported, 2));
            Assert.AreEqual(16963900000.00, Math.Round(deconv_components.OrderBy(c => c.id).First().intensity_sum, 2));
        }
    }
}
