...
namespace Test
{
    ...
    class TestPromex
    {
        ...
        public void testpromex()
        {
            ...
            string filepath = Path.Combine(Path.GetDirectoryName(Sweet.lollipop.input_files[0].complete_path), Path.GetFileNameWithoutExtension(Sweet.lollipop.input_files[0].complete_path) + "_deconv.xlsx");
            Assert.IsTrue(File.Exists(filepath));

            // Check contents of file to ensure nubmer of components match
            Sweet.lollipop.input_files.Clear();
            Sweet.lollipop.enter_input_files(new string[] { filepath }, Lollipop.acceptable_extensions[0], Lollipop.file_types[0], Sweet.lollipop.input_files, false);
